use crate::units::{Point3D, Real, Vector3D};
use cgmath::prelude::*;
...
pub trait Kernel3D {
    ...
    /// `r_sq`:     Squared length of ri_to_rj
    /// `r`:        Length of ri_to_rj
    fn gradient(&self, ri_to_rj: Vector3D, r_sq: Real, r: Real) -> Vector3D;

    #[inline(always)]
    fn gradient_from_positions(&self, ri: Point3D, rj: Point3D) -> Vector3D {
        let ri_to_rj = rj - ri;
