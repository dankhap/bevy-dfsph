...
pub struct DFSPHSolver3D<TViscosityModel: ViscosityModel> {
...
impl<TViscosityModel: ViscosityModel + std::marker::Sync> DFSPHSolver3D<TViscosityModel> {
    pub fn new(viscosity_model: TViscosityModel, smoothing_length: Real) -> DFSPHSolver3D<TViscosityModel> {
        DFSPHSolver3D {
            viscosity_model,
    ...
    fn compute_density_error(&self, dt: Real, fluid_world: &FluidParticleWorld3D, velocities: &[Vector3D], density_error: &mut [Real]) {
        microprofile::scope!("DFSPHSolver3D", "compute_density_error");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        ...
        density_error
            ...
            .for_each(|(i, (density_error_i, (&original_density, &pos_i, &velocity_vi)))| {
                ...
                if particles.boundary_particles.len() > 0 {
                particles.foreach_neighbor_particle_boundary(
                    i,
                    #[inline(always)]
                    |j| {
    ...
    fn correct_velocity_with_density_error(&mut self, dt: Real, fluid_world: &FluidParticleWorld3D, velocities: &mut [Vector3D], density_error: &[Real]) {
        microprofile::scope!("DFSPHSolver3D", "correct_velocity_with_density_error");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        let inv_dt = 1.0 / dt;
        ...
        velocities
            ...
            .for_each(|(i, ((predicted_velocity, warmstart_kappa_i), (&ri, density_error_i, alpha_i)))| {
                let mut delta: Vector3D = Zero::zero(); // gradient to self is zero.
    ...
    fn correct_density_error_warmstart(&self, dt: Real, fluid_world: &FluidParticleWorld3D, velocities: &mut [Vector3D]) {
        microprofile::scope!("DFSPHSolver3D", "correct_density_error_warmstart");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        let inv_dt = 1.0 / dt;
        let kernel = &self.kernel;
    ...
    fn correct_density_error(&mut self, dt: Real, fluid_world: &mut FluidParticleWorld3D, velocities: &mut [Vector3D]) {
        microprofile::scope!("DFSPHSolver3D", "correct_density_error");
        ...
        loop {
            microprofile::scope!("DFSPHSolver3D", "density_iteration");
    ...
    fn compute_density_change(&self, fluid_world: &FluidParticleWorld3D, velocities: &[Vector3D], density_change: &mut [Real]) {
        microprofile::scope!("DFSPHSolver3D", "compute_density_change");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        density_change
            .par_iter_mut()
    ...
    fn correct_velocity_with_divergence_error(&mut self, fluid_world: &FluidParticleWorld3D, velocities: &mut [Vector3D], density_change: &[Real]) {
        microprofile::scope!("DFSPHSolver3D", "correct_velocity_with_divergence_error");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        let kernel = &self.kernel;
        let alpha_values = &self.alpha_values;
    ...
    fn correct_divergence_error_warmstart(&self, fluid_world: &FluidParticleWorld3D, velocities: &mut [Vector3D]) {
        microprofile::scope!("DFSPHSolver3D", "correct_divergence_error_warmstart");
        let particle_mass = fluid_world.properties.particle_mass();
        let particles = &fluid_world.particles;
        let kernel = &self.kernel;
    ...
    fn correct_divergence_error(&mut self, dt: Real, fluid_world: &mut FluidParticleWorld3D, velocities: &mut [Vector3D]) {
        microprofile::scope!("DFSPHSolver3D", "correct_divergence_error");
        ...
        loop {
            microprofile::scope!("DFSPHSolver3D", "density_change_iteration");
...
impl<TViscosityModel: ViscosityModel + std::marker::Sync> Solver3D for DFSPHSolver3D<TViscosityModel> {
    ...
    fn simulation_step(&mut self, fluid_world: &mut FluidParticleWorld3D, time_manager: &mut TimeManager) {
        microprofile::scope!("DFSPHSolver3D", "simulation_step");
        ...
        // compute non-pressure forces (from scratch)
        {
            ...
            {
                microprofile::scope!("DFSPHSolver3D", "non-pressure forces");

                let non_pressure_forces: Vector3D = fluid_world.gravity * fluid_world.properties.particle_mass();
                let particle_mass = fluid_world.properties.particle_mass();
            ...
            // update timestep
            {
                microprofile::scope!("DFSPHSolver3D", "update timestep");
                let dt = time_manager.timestep();
            ...
            // predict velocity
            {
                microprofile::scope!("DFSPHSolver3D", "velocity prediction");
                let dt = time_manager.timestep();
