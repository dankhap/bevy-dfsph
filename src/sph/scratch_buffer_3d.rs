...
pub struct ScratchBufferStore {
    ...
    buffers_vector: Arc<Mutex<ScratchBufferTypeStore<Vector3D>>>,
    buffers_idx: Arc<Mutex<ScratchBufferTypeStore<f64>>>,
}
...
impl ScratchBufferStore {
    pub fn new() -> ScratchBufferStore {
        ScratchBufferStore {
            ...
            buffers_idx: Arc::new(Mutex::new(ScratchBufferTypeStore::new())),
        }
    }
    ...
    pub fn get_buffer_uint(&self, size: usize) -> ScratchBuffer<u64, f64> {
        ScratchBuffer::<u64, f64> {
            buffer: self.buffers_idx.lock().unwrap().get_buffer(size, 0 as u64),
            store: Arc::clone(&self.buffers_idx),
        }
    }
    ...
    pub fn get_buffer_vector(&self, size: usize) -> ScratchBuffer<Vector3D, Vector3D> {
        ScratchBuffer::<Vector3D, Vector3D> {
            buffer: self.buffers_vector.lock().unwrap().get_buffer(size, Vector3D::zero()),
            store: Arc::clone(&self.buffers_vector),
        }
    }

    pub fn get_buffer_point(&self, size: usize) -> ScratchBuffer<Point3D, Vector3D> {
        ScratchBuffer::<Point3D, Vector3D> {
            buffer: self.buffers_vector.lock().unwrap().get_buffer(size, Point3D::origin()),
            store: Arc::clone(&self.buffers_vector),
        }
    }
}
