...
pub use encode_bitfiddle as encode;
...
// Decodes y part of 2d morton code.
pub use decode_z_bitfiddle as decode_z;
...
pub(super) fn is_in_rect(m_cur: u64, min_morton: u64, max_morton: u64) -> bool {
    ...
    let min_morton_zbits = min_morton & MORTON_ZBITS;
    is_in_rect_presplit(m_cur, min_morton_xbits, min_morton_ybits, min_morton_zbits, 
                               max_morton_xbits, max_morton_ybits, max_morton_zbits)
}
...
pub(super) fn is_in_rect_presplit(m_cur: u64, min_morton_xbits: u64, min_morton_ybits: u64, min_morton_zbits: u64, 
    ...
    cur_x >= min_morton_xbits && cur_y >= min_morton_ybits && cur_z >= min_morton_zbits &&
    cur_x <= max_morton_xbits && cur_y <= max_morton_ybits && cur_z <= max_morton_zbits 
}
