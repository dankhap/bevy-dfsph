...
impl MortonCellPos {
    ...
    fn to_cidx(self) -> MortonCellIndex {
        super::morton_3d::encode(self.x, self.y, self.z)
    }

    #[inline]
    fn from_cidx(cidx: MortonCellIndex) -> MortonCellPos {
        MortonCellPos {
            x: super::morton_3d::decode_x(cidx) as u16,
            y: super::morton_3d::decode_y(cidx) as u16,
            z: super::morton_3d::decode_z(cidx) as u16,
        }
    }
}
...
impl CompactMortonCellGrid {
    ...
    pub fn update(
        ...
        for (i, &pos) in positions.iter().enumerate() {
            ...
            /* if cidx == 0{
                println!("got cidx 0 in neighborhood_search_3d::106, i={}", i);
                println!("pos: {:#?}", pos);
            } */
            /* if cidx == 40210710958665{
                println!("got bad x pos in update, i={}", i);
                println!("pos: {:#?}", pos);
            }
 */
    ...
    fn get_particle_runs_in_neighborbox(&self, cidx: MortonCellIndex) -> MortonCellNeihborhoodRuns {
        ...
        // println!("geting particles in {:?}, for {}", pos, cidx);
        let cidx_min = MortonCellPos { x: pos.x - 1, y: pos.y - 1, z: pos.z - 1 }.to_cidx();
        let cidx_max = MortonCellPos { x: pos.x + 1, y: pos.y + 1, z: pos.z + 1 }.to_cidx();

        let cidx_min_xbits = cidx_min & super::morton_3d::MORTON_XBITS;
        let cidx_min_ybits = cidx_min & super::morton_3d::MORTON_YBITS;
        let cidx_min_zbits = cidx_min & super::morton_3d::MORTON_ZBITS;
        let cidx_max_xbits = cidx_max & super::morton_3d::MORTON_XBITS;
        let cidx_max_ybits = cidx_max & super::morton_3d::MORTON_YBITS;
        let cidx_max_zbits = cidx_max & super::morton_3d::MORTON_ZBITS;
        ...
        while cell.cidx <= cidx_max {
            ...
            while !super::morton_3d::is_in_rect_presplit(cell.cidx, cidx_min_xbits, cidx_min_ybits, cidx_min_zbits,
                                                                 cidx_max_xbits, cidx_max_ybits, cidx_max_zbits) {
                num_misses += 1;
            ...
            loop {
                ...
                cell = self.cells[cell_arrayidx];
                if !super::morton_3d::is_in_rect_presplit(cell.cidx, cidx_min_xbits, cidx_min_ybits, cidx_min_zbits,
                                                                  cidx_max_xbits, cidx_max_ybits, cidx_max_zbits) {
                    break;
                }
            }
    ...
    pub fn foreach_potential_neighbor(&self, grid: &GridProperties, position: Point3D, mut f: impl FnMut(usize) -> ()) {
        println!("getting particle runs in 265");
